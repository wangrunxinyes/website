...
class ETinyMce extends CInputWidget
{
   ...
   *
   ...
   /**
    * The convert_urls attribute of the editor.
    *
    * @var boolean
    */
   private $convertUrls = true;
    ...
    * @var array
    */
   private $validEditorTemplates = array('simple', 'full','footer','header','alterfooter', 'webform');
   ...
   /**
    * Will we use the convertUrl?
    *
    * @return boolean
    */
   public function getConvertUrls()
   {
    ...
    * @param boolean $value
    */
   public function setConvertUrls($value)
   {
   	if (!is_bool($value))
   		throw new CException(Yii::t('ETinyMCE', 'useSwitch must be boolean'));
   	$this->convertUrls = $value;
   }
   ...
   protected function makeFullEditor($url='')
   {
      ...
      $this->plugins = array('safari','pagebreak','style','layer','table','save','advhr','filemanager','advimage','advlink','emotions','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','contextmenu','paste','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template');
      ...
      $options['extended_valid_elements'] =  'style[type],+input[*|placeholder],+textarea[*|placeholder],+select[*|placeholder]';
      ...
      //$options['theme_advanced_path_location'] = 'bottom';
		...
		$options['theme_advanced_buttons1'] = "newdocument,|,styleprops,spellchecker,|,search,replace,|,table,tablecontrols,|,bullist,numlist,|,outdent,indent,link,unlink,cleanup,help,code,|,insertdate,inserttime,|,fullscreen";
		...
		$options['theme_advanced_buttons2'] = "cut,copy,paste,pastetext,pasteword,|,insertfile,image,|,undo,redo,|,blockquote,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,formatselect,fontselect,fontsizeselect,|,forecolor,backcolor";
   ...
   /** Editor for webform **/
   protected function makeWebFormEditor($url='')
   {
   	$options = array();
   
   	$this->plugins = array('safari','pagebreak','style','layer','table','save','advhr','filemanager','advimage','advlink','emotions','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','contextmenu','paste','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template', 'fullpage');
   ...
   protected function makeFooterEditor($url='')
   {
   	$options = array();
   	...
   	$options['theme_advanced_toolbar_align'] = 'left';
   	//$options['theme_advanced_path_location'] = 'bottom';
   	$options['theme_advanced_buttons1'] = "bold,italic,underline,strikethrough,justifyleft,justifycenter,justifyright,fontselect,fontsizeselect,forecolor,backcolor,code";
   	$options['theme_advanced_buttons2'] = "";
   	$options['theme_advanced_buttons3'] = "";
   	$options['theme_advanced_buttons4'] = "";
   	$options['theme_advanced_resize_horizontal'] = false;
   	$options['theme_advanced_resize_vertical'] = true;
   	$options['theme_advanced_resizing'] = false;
   	$options['resize'] = 'both';
   	$options['spellchecker_languages'] = '+English=en,Danish=da,Dutch=nl,Finnish=fi,French=fr,German=de,Italian=it,Polish=pl,Portuguese=pt,Spanish=es,Swedish=sv';
   	if ($url !== '') {
   ...
   protected function makeAlterFooterEditor($url='')
   {
   	$options = array();
   ...
   protected function makeHeaderEditor($url='')
   {
   	$options = array();
   ...
   protected function makeCompressor()
   {
	...
	protected function makeOptions($url='')
	{
      ...
      // avoid URL headaches
      $options['convert_urls'] = $this->convertUrls;
      ...
      if ($this->editorTemplate !== '') {
         switch ($this->editorTemplate) {
            ...
            case 'full':
               ...
               break;
            case 'webform':
               	$template = $this->makeWebFormEditor($url);
               	break;               
            case 'footer':
               $template = $this->makeFooterEditor($url);
            case 'alterfooter':
               $template = $this->makeAlterFooterEditor($url);               
            case 'header':
               	$template = $this->makeHeaderEditor($url);               
               break;               
         }
